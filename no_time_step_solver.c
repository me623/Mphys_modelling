...
void malloc_Sim_Species_arrays(SimulationParams *Sim)
{
    ...
    *Sim->buffer_index = 0;
    ...
    for (int32_t i = 0; i < Sim->n_species; i++)
    {
        ...
        Sim->Species[i]->n = calloc(Sim->array_len + 1, sizeof(double));
    }
}
...
void iteration(SimulationParams *Sim, LeptonParams *Lepton)
{
    ...
    {
        Lepton->n[i+1] =
        Lepton->delta_gamma[i] * I(Lepton->gamma[i], Sim) +
        Lepton->n[i] * (1 - Lepton->delta_gamma[i] / Sim->tau_esc);
    }
}
...
int main()
{
    ...
    FILE *file = fopen("simulation_data_2.csv", "w");
