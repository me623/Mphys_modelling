...
void iteration(SimulationParams *Sim, LeptonParams *Lepton)
{
    for (int64_t i = Sim->array_len; i >= 0; i--)
    {
        ...
        - Lepton->delta_gamma[i] * Sim->dt * I(Lepton->gamma[i], Sim)
...
void set_initial_state(SimulationParams *Sim)
{
    ...
    for (int32_t lepton = 0; lepton < Sim->n_species; lepton++)
    {
        ...
        {
            Sim->Species[lepton]->current_n[i] =(Sim->rho / m_e) * (I(Sim->Species[lepton]->gamma[i], Sim) / Sim->Q_e0);
        }
    }
}
...
int main()
{
    ...
    printf("norm_f%e, Q_e0%e, mean_g%e, V%e, c%e \n", Sim->norm, Sim->Q_e0, average_gamma, volume, c);
