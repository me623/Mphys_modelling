...
void malloc_and_fill_gamma_array(SimulationParams *Sim, ElectronParams *Electrons)
{
    ...
    decades = log10(Sim->max_gamma) - log10(Sim->min_gamma);
...
{
    double gamma;
    double rho = pow(10., -21.96);
    double nu = 0.;
    double A = (Sim->inject_power - 1.) * pow(Sim->inject_min, (Sim->inject_power - 1.));
    double prefactor = (rho / (m_e + nu * m_p)) * A;

    for (int64_t i = 0; i < Sim->array_len; i++)
    {
        gamma = Electron->gamma[i];
        if (gamma < Sim->inject_break && gamma > Sim->inject_min)
        {
            Electron->n[i] = pow(gamma, - Sim->inject_power);
        }
        else
        {
            if (gamma > Sim->inject_break && gamma < Sim->inject_max)
            {
                Electron->n[i] = pow(Sim->inject_break, Sim->inject_power_2 - Sim->inject_power) * pow(gamma, - Sim->inject_power_2);
            }
            else
            {
                Electron->n[i] = 0.;
...
void write_run_file(SimulationParams *Sim, char *filename)
{
    ...
    fprintf(file, 
    "delta_ln_gamma,R,inject_p,inject_p_2,inject_min,inject_break,inject_max,B,L,end_tol,Q_e0,S,tau_esc,norm,avg_gamma,V,array_len,max_gamma,min_gamma,init_p,samples_per_decade,change,crit_freq,tau_acc,iter,gamma_eq,gamma_eq_break,whole_t,stepping_t\n");
    fprintf(file,
    "%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%lld,%e,%e,%e,%lld,%e,%e,%e,%lld,%e,%lld,%e,%e,\n",
    Sim->Electrons->delta_ln_gamma, Sim->R,Sim->inject_power, Sim->inject_power_2,Sim->inject_min,Sim->inject_break,Sim->inject_max,Sim->B,Sim->L,
...
int main()
{
    ...
    */
    ...
    char filename[100], filepath[100];
    sprintf(filename, "fixed_n_e.csv");
    sprintf(filepath, "csv_data/steady_state/%s", filename);
    FILE *file = fopen(filepath, "w");
    fclose(file);
